...
class Header_user_request_admin(admin.ModelAdmin):
	...
	def save_model(self, request, Header_user_request,form,change):
		...
		if dep.name == 'unit':
			Header_user_request.department = dep.department
	...
	def get_form(self, request, obj=None, **kwargs):
		...
		if data.name == 'unit':
			...
			if x == False:
				form.base_fields['user'].queryset = form.base_fields['user'].queryset.filter(department=data.department)
			else : 
	...
	def queryset(self, request, obj=None):
		...
		if user.name=='unit':
			return Header_user_request.objects.filter(department=user.department)
		else :
	...
	def get_readonly_fields(self, request, obj=None):
		...
		if data.name == 'unit':
			...
			if getattr(obj, 'department_staff_aggreement', None) == True:
				readonly_fields = ('department','department_staff_aggreement','user',) 	
			
				
		elif data.name == 'staff':
			readonly_fields += ('no_reg','department','department_staff_aggreement','user',)
...
class Data_user_request_admin(admin.ModelAdmin):
	...
	def get_form(self, request, obj=None, **kwargs):
		...
		if data.name == 'unit':
			...
			if xx == False:
				form.base_fields['asset'].queryset = form.base_fields['asset'].queryset.filter(department=data.department)
				...
				form.base_fields['header'].queryset = form.base_fields['header'].queryset.filter(department=data.department)
	...
	def queryset(self, request, obj=None):
		user = Group.objects.get(user=request.user)
		if user.name=='unit':
			return Data_user_request.objects.filter(header__department=user.department)
