...
class Header_request_asset_admin(admin.ModelAdmin):
	...
	def queryset(self, request, obj=None):
		...
		if user.name=='unit':
			return Header_request_asset.objects.filter(department=user2.employee.department)
		else :
...
class Data_request_asset_admin(admin.ModelAdmin):
	...
	def queryset(self, request, obj=None):
		...
		user2 = StaffPerson.objects.get(user=request.user)
		if user.name=='unit':
			return Data_request_asset.objects.filter(header_request__department=user2.employee.department)
		else :
