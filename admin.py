...
class CustomStaffAdmin(admin.ModelAdmin):
    add_form_template = 'admin/auth/user/add_form.html'
    change_user_password_template = None
    fieldsets = (
        (None, {'fields': ('username', 'password', 'email', 'is_active', 'is_staff',)}),
    ...
    def get_fieldsets(self, request, obj=None):
        if not obj:
            return self.add_fieldsets
        return super(CustomStaffAdmin, self).get_fieldsets(request, obj)
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        defaults.update(kwargs)
        return super(CustomStaffAdmin, self).get_form(request, obj, **defaults)
    ...
    def get_urls(self):
        ...
        return patterns('',
            (r'^(\d+)/password/$',
             self.admin_site.admin_view(self.user_change_password))
        ) + super(CustomStaffAdmin, self).get_urls()

    def lookup_allowed(self, lookup, value):
        # See #20078: we don't want to allow any lookups involving passwords.
        if lookup.startswith('password'):
            return False
        return super(CustomStaffAdmin, self).lookup_allowed(lookup, value)
    ...
    def add_view(self, request, form_url='', extra_context=None):
        ...
        extra_context.update(defaults)
        return super(CustomStaffAdmin, self).add_view(request, form_url,
                                               extra_context)
    ...
    def response_add(self, request, obj, post_url_continue=None):
        ...
        if '_addanother' not in request.POST and IS_POPUP_VAR not in request.POST:
            request.POST['_continue'] = 1
        return super(CustomStaffAdmin, self).response_add(request, obj,
                                                   post_url_continue)