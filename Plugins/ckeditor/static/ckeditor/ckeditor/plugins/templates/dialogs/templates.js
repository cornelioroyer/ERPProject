    ...
    CKEDITOR.dialog.add("templates", function (c) {
        ...
        function i(a) {
            ...
            if (c || g.contains(b)) {
                ...
                if (f) {
                    if (c)e = f.getItem(0); else switch (d) {
                        case 40:
                            e = b.getNext();
                            break;
                        case 38:
        ...
        return{title: c.lang.templates.title,
            minWidth: CKEDITOR.env.ie ? 440 : 400, minHeight: 340, contents: [
                {id: "selectTpl", label: f.title, elements: [
                    {type: "vbox", padding: 5, children: [
                        {id: "selectTplText", type: "html", html: "<span>" + f.selectPromptMsg + "</span>"},
                ...
                CKEDITOR.loadTemplates(l.templates_files, function () {
                    ...
                    if (b.length) {
                        var c = g;
                        c.setHtml("");
                        for (var d = 0, h = b.length; d < h; d++)for (var e = CKEDITOR.getTemplates(b[d]), i = e.imagesPath, e = e.templates, n = e.length, j = 0; j < n; j++) {
                            var m = o(e[j], i);
