...
from Plugins.captcha.conf import settings as captcha_settings
...
class CaptchaStore(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.hashkey:
            key_ = unicodedata.normalize('NFKD', str(randrange(0, MAX_RANDOM_KEY)) + str(time.time()) + six.text_type(
                self.challenge)).encode('ascii', 'ignore') + unicodedata.normalize('NFKD',
                                                                                   six.text_type(self.response)).encode(
                'ascii', 'ignore')
            if hashlib:
