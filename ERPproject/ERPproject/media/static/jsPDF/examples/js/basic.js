...
function printreport() {
    ...
    pdf.fromHTML(
    	...
    	function (dispose) {
    	  // dispose: object with X, Y of the last line add to the PDF 
    	  //          this allow the insertion of new lines after html
          pdf.save('Dokumen PT IGLAS PERSERO.pdf');
        },
    	margins
    )
}
...
function reportgl() {
    ...
    //doc.setFontSize(22), doc.text(20,20,'This is a title'), doc.setFontSize,  podo
	// source can be HTML-formatted string, or a reference
	// to an actual DOM element from which the text will be scraped.
	, source = $('#divsubledger')[0]
    ...
    // 'inches' in this case
    pdf.fromHTML(
    	source // HTML string or DOM elem ref. ($('body').get(0),15,15,{eleedoc.setFontSize(22)
    	, margins.left // x coord 15,
    	, margins.top // y coord 15,
    	, {
    		'width': margins.width // max width of content on PDF 
    		, 'elementHandlers': specialElementHandlers
    	},
...
function reportbalance() {
    ...
    pdf.fromHTML(
    	...
    	function (dispose) {
    	  // dispose: object with X, Y of the last line add to the PDF 
    	  //          this allow the insertion of new lines after html
          pdf.save('reportbalance');
        },
    	margins
    )
}

function reportlossprofit() {
	var pdf = new jsPDF('l', 'pt', 'a4')
    //doc.setFontSize(22), doc.text(20,20,'This is a title'), doc.setFontSize,  podo
	// source can be HTML-formatted string, or a reference
	// to an actual DOM element from which the text will be scraped.
	, source = $('#divlossprofit')[0]
